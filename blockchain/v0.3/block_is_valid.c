	...
	llist_t *all_unspent)
{
	...
	block_t *_genesis = calloc(1, sizeof(*_genesis));
	_genesis->info.timestamp = GENESIS_TIMESTAMP;
	memcpy(&(_genesis->data.buffer), GENESIS_DATA, GENESIS_DATA_LEN);
	_genesis->data.len = GENESIS_DATA_LEN;
	memcpy(&(_genesis->hash), GENESIS_HASH, SHA256_DIGEST_LENGTH);
